...
#define MAX_MSG_LENGTH 50  // Ajusta este valor si es necesario

char spi_tx_buf[MAX_MSG_LENGTH + 1];  // Buffer para el mensaje recibido
char uart_buf[100];  // Buffer para mensajes de UART
int uart_buf_len;
uint8_t msg_length;
uint8_t msg_length_digits[2];  // Para almacenar los dos dígitos del primer byte
int slave_selector;  // Variable para seleccionar el slave
